...
public class AStarEuclid {

    char[][] maze;
    public AStarEuclid(char[][] maze) {
        this.maze = maze;
    ...
    private static int calculateHeuristic(int[] current, int[] goal) {
        double x = Math.abs(current[0] - goal[0]);
        double y = Math.abs(current[1] - goal[1]);
        x = x*x;
        y = y*y;
        ...
        while (!pq.isEmpty()) {
            ...
            for (int[] dir : DIRECTIONS) {
                ...
                if (tentativeGCost < gCostMap.getOrDefault(neighborKey, Integer.MAX_VALUE)) {
                    ...
                    {
                        visitedCells.add(new int[]{current.current[0],current.current[1]});
